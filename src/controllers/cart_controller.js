...
export const updateCartWithProducts = async (req,res,next) => {
  try {
    ...
    const cartExist = await service.getCartById(cid);
    if (!cartExist) return res.status(404).json({ msg: "Cart not found" }); //if the cart doesn't exist on db
    const updateCartWithProd = await service.updateCartWithProducts(cid, req.body);
...
export const updateProductQuantity = async (req,res,next) => {
  try {
    ...
    const cartExist = await service.getCartById(cid);
    if (!cartExist) return res.status(404).json({ msg: "Cart not found" }); //if the cart doesn't exist on db
    const productExistInCart = cartExist.products.find((prod) => prod.product._id.toString() === pid);
    if (!productExistInCart) return res.status(404).json({ msg: "Product doesn't exist in the cart" }); //if the product doesn't exist in the cart
...
export const deleteAllProductsFromCart = async (req, res, next) => {
  try {
    ...
    if (productsExistInCart === 0) return res.status(400).json({ msg: "Cart is empty" });
    const deleteAllProductsFromCart = await service.deleteAllProductsFromCart(cid);
